...
O2Slopes <- O2SlopeData %>%
  ...
  mutate(O2lm = map(O2data,  possibly(~ lm(O2_umolL ~ as.numeric(AbsTime), data = .x), otherwise = NULL)),
  ...
  mutate(O2evln_O2ls = (O2_umolLs - O2_umolLs[5]) * 6.022e17) %>%
  ungroup()

O2Slopes2 <- O2SlopeData %>%
  filter(Dark1sinter == 0) %>%
  nest(O2data = -c(Date, CultureID, Ex_WL, filenameO2, ActPARinter, LR_sFRRfinter)) %>% 
  mutate(O2data2 = map(O2data, ~slice_tail(., n = 145)), #keep last 145 rows of nested data
    O2lm = map(O2data2,  possibly(~ lm(O2_umolL ~ as.numeric(AbsTime), data = .x), otherwise = NULL)),
         O2lm_tidy = map(O2lm, possibly(tidy, otherwise = NULL))) %>%
  select(-c(O2data, O2data2, O2lm)) %>%
  unnest(cols = O2lm_tidy) %>%
  filter(term  != "(Intercept)") %>%
  select(-term) %>%
  rename(O2_umolLs = estimate) %>%
...
O2FRRfMetaNest %>%
  ...
  geom_point(aes(x = O2evln_O2ls, y = .resid), colour = "darkred") +
...
```{r save O2FRRfMeta}
saveRDS(O2FRRfMeta, file.path(DataOut, paste(Project, "O2FRRfMeta.Rds", sep = "_"), fsep = .Platform$file.sep))
```
